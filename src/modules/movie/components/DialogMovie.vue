<template>
  ...
  >
    <v-card class="movie-card" elevation="24">
      <v-img
        :src="controller.selectedMovie.value.poster_path"
        height="400px"
        class="movie-poster"
      >
          ...
          <v-row>
            <v-col cols="12" class="synopsis-section">
              <p class="movie-synopsis">
                {{
                  controller.selectedMovie.value.synopsis ||
            ...
            <v-col cols="12" sm="6" class="movie-info">
              ...
              {{ controller.selectedMovie.value.director || "Desconhecido" }}
            </v-col>
            <v-col cols="12" sm="6" class="movie-info">
              <v-icon color="blue" class="mr-2">mdi-clock-outline</v-icon>
              <strong>Duração:</strong>
              {{
                controller.selectedMovie.value.duration || "Não disponível"
              }}
              min
            </v-col>
            <v-col cols="12" sm="6" class="movie-info">
              ...
              {{ controller.selectedMovie.value.year || "Data não disponível" }}
            </v-col>
            <v-col cols="12" sm="6" class="movie-info">
              <v-icon color="red" class="mr-2">mdi-heart</v-icon>
              <strong>Favorito:</strong>
              {{ controller.selectedMovie.value.favorite ? "Sim" : "Não" }}
            </v-col>
          </v-row>
        </v-container>
      </v-card-text>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" @click="controller.closeDialog()" elevation="5"
          >Fechar</v-btn
        >
      </v-card-actions>
    </v-card>
