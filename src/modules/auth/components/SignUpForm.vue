<template>
  <v-form
    ref="registerForm"
    v-model="controller.valid.value"
    lazy-validation
    @submit.prevent
  >
    <v-text-field
    ...
    <v-text-field
      ...
      type="password"
      :rules="[
        (v) => controller.isPasswordMatching.value || 'As senhas não coincidem',
      ]"
      required
      hide-details="auto"
    ></v-text-field>

    <v-btn class="mt-4" color="success" block @click="controller.register()" type="submit"
      >Cadastrar</v-btn
    >
  </v-form>
</template>
